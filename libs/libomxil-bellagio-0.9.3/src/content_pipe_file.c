...
CPresult file_pipe_Constructor(CP_PIPETYPE **ppPipe, CPstring szURI)
{
    ...
    pFilePipe = (file_ContentPipe*) malloc(sizeof(file_ContentPipe));
    if (!pFilePipe)
        return 0;

    memset(pFilePipe, 0x00, sizeof(file_ContentPipe));
