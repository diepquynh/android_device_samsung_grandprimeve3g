...
OMX_ERRORTYPE base_image_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
    ...
    if (!(*openmaxStandPort)) {
        *openmaxStandPort = malloc(sizeof(omx_base_image_PortType));
        if (!(*openmaxStandPort)) {
            DEBUG(DEB_LEV_ERR, "Insufficient memory in %s\n", __func__);
            return OMX_ErrorInsufficientResources;
        }
        memset(*openmaxStandPort, 0x00, sizeof(omx_base_image_PortType));
    }
    ...
    if (!omx_base_image_Port->sPortParam.format.image.cMIMEType) {
        DEBUG(DEB_LEV_ERR, "Insufficient memory in %s\n", __func__);
        return OMX_ErrorInsufficientResources;
    }
