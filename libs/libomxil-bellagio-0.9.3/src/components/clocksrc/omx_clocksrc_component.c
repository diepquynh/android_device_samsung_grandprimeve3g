...
OMX_ERRORTYPE omx_clocksrc_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
  ...
  if (!openmaxStandComp->pComponentPrivate) {
    ...
    openmaxStandComp->pComponentPrivate = malloc(sizeof(omx_clocksrc_component_PrivateType));
    if(openmaxStandComp->pComponentPrivate==NULL) {
      return OMX_ErrorInsufficientResources;
    }
    memset(openmaxStandComp->pComponentPrivate, 0x00, sizeof(omx_clocksrc_component_PrivateType));
  }
  ...
  if (omx_clocksrc_component_Private->sPortTypesParam[OMX_PortDomainOther].nPorts && !omx_clocksrc_component_Private->ports) {
    ...
    omx_clocksrc_component_Private->ports = malloc(omx_clocksrc_component_Private->sPortTypesParam[OMX_PortDomainOther].nPorts*sizeof(omx_base_PortType *));
    if (!omx_clocksrc_component_Private->ports) {
      return OMX_ErrorInsufficientResources;
    }
    memset(omx_clocksrc_component_Private->ports, 0x00, omx_clocksrc_component_Private->sPortTypesParam[OMX_PortDomainOther].nPorts*sizeof(omx_base_PortType *));
    ...
    for (i=0; i < omx_clocksrc_component_Private->sPortTypesParam[OMX_PortDomainOther].nPorts; i++) {
      ...
      omx_clocksrc_component_Private->ports[i] = malloc(sizeof(omx_base_clock_PortType));
      
      if (!omx_clocksrc_component_Private->ports[i]) {
        return OMX_ErrorInsufficientResources;
      }
      
      memset(omx_clocksrc_component_Private->ports[i], 0x00, sizeof(omx_base_clock_PortType));
  ...
  if(!omx_clocksrc_component_Private->clockEventSem) {
    ...
    omx_clocksrc_component_Private->clockEventSem = malloc(sizeof(tsem_t));
    memset(omx_clocksrc_component_Private->clockEventSem, 0x00, sizeof(tsem_t));
  ...
  if(!omx_clocksrc_component_Private->clockEventCompleteSem) {
    ...
    omx_clocksrc_component_Private->clockEventCompleteSem = malloc(sizeof(tsem_t));
    memset(omx_clocksrc_component_Private->clockEventCompleteSem, 0x00, sizeof(tsem_t));
